...
WriteComm(0x20);     //Display Inversion On

WriteComm(0x3A);     
WriteData(0x05);   //262k colors

WriteComm(0xB7);     //VGH=13.26 VGL=-10.43
WriteData(0x35);   

WriteComm(0xBB);     //VCOM=1.15V
WriteData(0x06); //06  //2b

WriteComm(0xC0);     //LCM Control
WriteData(0x2C); 

WriteComm(0xC2);     //VRH=Command Enable
WriteData(0x01); 
WriteData(0xFF); 

WriteComm(0xC3);     //Vop=4.5V=0x13
WriteData(0x13);   

WriteComm(0xC6);			
WriteData(0x0F); //DOT0F CO7F

//#ifdef INV_DOT										
//		WriteComm(0xC6);			WriteData(0x0F); //0x09orig [DB7-5] 0forDotInv,1forColumnInv; 
//		                                               //[DB4-0] Frame Rate,0x0F:60Hz
//#else
//		WriteComm(0xC6);			WriteData(0xEF); //0x09orig [DB7-5] 0forDotInv,1forColumnInv; 
//		                                               //[DB4-0] Frame Rate,0x0F:60Hz
//#endif
...
WriteComm(0xE0);     
WriteData(0x70);  // V63&V0 
WriteData(0x27);  // V1  
WriteData(0x28);	// V2   
WriteData(0x07);   // V4
WriteData(0x04);   // V6 04
WriteData(0x27);   //V13 26
WriteData(0x38); 	//V20 44 39  
WriteData(0x33);	//V27 54   
WriteData(0x56);   //V43 56
WriteData(0x3A);  //V50 3A 
WriteData(0x13);   
WriteData(0x10);   
WriteData(0x25);   
WriteData(0x27);  
...
WriteData(0x00);	// V4   
WriteData(0x00);	// V6   
WriteData(0x09);	// V13  //00 09
WriteData(0x44);	// V20 //44  39
WriteData(0x47);	// V27  //55 4A  51
WriteData(0x4C);	// V43  //57
WriteData(0x0D);	// V50 //0F
WriteData(0x1D);	// V57   //1D
WriteData(0x19);	// V59   //1C
WriteData(0x20);	// V61   //2A
WriteData(0x25);	// V62	//2D
...
/*	WriteComm(0x11); // 2: Out of sleep mode, no args, w/delay
	delay_ms(120);

	WriteComm(0x2A);// 3: Set color mode, 16-bit color
	WriteData(0x55); 
	...
	WriteComm(0x13); // 4: Normal display on, no args, w/delay
	delay_ms(10);

	WriteComm(0x29);// 5: Main screen turn on, no args, w/delay
*/
